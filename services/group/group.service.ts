...
class GroupService {
  ...
  addVisit_log(group_id: string, visit_log: Partial<IGroup>): Promise<void> {
    const GroupModel = model<GroupType>(DatabaseTablesEnum.GROUP_COLLECTION_NAME, GroupSchema);

    return GroupModel.findByIdAndUpdate(group_id, {$set: {attendance: visit_log}}) as any;
  }

  getStudentsList(group_id: string): Promise<Partial<IGroup>> {
    const GroupModel = model<GroupType>(DatabaseTablesEnum.GROUP_COLLECTION_NAME, GroupSchema);
  ...
  getVisitLog(group_id: string, date?: Partial<IGroup>): Promise<Partial<IGroup>> {
    const GroupModel = model<GroupType>(DatabaseTablesEnum.GROUP_COLLECTION_NAME, GroupSchema);
